...
def hello(name):
    ...
    if day_time < 12:
        speak("Chào buổi sáng bạn{}. Chúc bạn một ngày vui vẻ.".format(name))
    elif 12 <= day_time < 18:
        speak("Chào buổi chiều bạn{}. Bạn đã dự định cho chiều nay chưa.".format(name))
    else:
...
def get_time(text):
    ...
    if "giờ" in text:
        ...
        speak("Hôm nay là ngày %d tháng %d năm %d"%(now.date,now.month,now.year))
    else:
...
def assistant():
    ...
    if name:
        ...
        while True:
            ...
            if not text:
                break
            elif "dừng" in text or "tạm biệt bot" in text or "đi ngủ" in text:
                stop()
                break
